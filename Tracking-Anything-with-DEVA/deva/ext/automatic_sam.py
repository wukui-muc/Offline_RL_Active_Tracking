...
def calculate_sum_distance(bboxes, fixed_point):
    sum_distances = []
    for bbox in bboxes:
        x1, y1, x2, y2 = bbox
        corners = [(x1, y1), (x1, y2), (x2, y1), (x2, y2)]
        sum_distance = sum(calculate_distance(corner, fixed_point) for corner in corners)
...
def auto_segment(config: Dict, auto_sam: SamAutomaticMaskGenerator, image: np.ndarray,
    ...
    #calculate the position of bbox to define the target id
    global flag
    # if flag==0:
    bbox= np.array(mask_data['boxes'].cpu())
    dist_to_mid=calculate_sum_distance(bbox,(h/2,w/2))
    target_id =np.argmin(dist_to_mid)
    flag=1
    ...
    if pred_masks.shape[0] == 0:
        ...
        if suppress_small_mask:
            ...
            for k in range(pred_masks.shape[0]):
                ...
                # mask = mask.cpu()
                # mask = np.array(mask)
                # cv2.imwrite('tmp_mask.jpg', mask / mask.max() * 255)
                if mask.sum() > 0:
                    segments_info.append(ObjectInfo(id=curr_id, score=predicted_iou[k].item()))
                    curr_id += 1
                    print(curr_id)

    return output_mask, segments_info,bbox,target_id
