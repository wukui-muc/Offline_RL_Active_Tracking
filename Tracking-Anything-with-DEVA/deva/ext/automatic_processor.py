...
from deva.inference.result_utils import save_result
def make_segmentation(cfg: Dict, image_np: np.ndarray, forward_mask: Optional[torch.Tensor],
    ...
    mask, segments_info,bbox,target_id = auto_segment(cfg, sam_model, image_np, forward_mask, min_side,
                                       suppress_small_mask)
    return mask, segments_info,bbox,target_id
...
def process_frame_automatic(deva: DEVAInferenceCore,
    ...
    if cfg['temporal_setting'] == 'semionline':
        ...
        if ti % cfg['detection_every'] == 0:
            ...
            mask, segments_info,bbox,target_id = make_segmentation(cfg, image_np, forward_mask, sam_model,
                                                    new_min_side, suppress_small_mask)
            frame_info.segments_info = segments_info
